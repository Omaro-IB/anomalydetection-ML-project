...
def predictor(hourOfDay, dayOfWeek, monthOfYear, meter_reading, lag1, lag2, lag3, listOfModels, n):
    """
    ...
    :param listOfModels: List; list containing RFR models to consider using
    :param n: Number of random models to use
    :return: Float: 0 to 1 representing whether the features listed is an anomaly or not based on the random models' consensus
    """
    shuffle(listOfModels)
    modelsToUse = listOfModels[:n]

    votes = []
    for model in modelsToUse:
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            votes.append(train.predict(model, [hourOfDay, dayOfWeek, monthOfYear, meter_reading, lag1, lag2, lag3]))

    return (sum(votes) / len(votes))[0]

def processDF(df, listOfModels, n):
    """
    ...
    :param listOfModels: list containing RFR models to consider using
    ...
    for i,j in df.iterrows():
        ...
        if True in pd.isna(currRow):
            break
        anomalies.append(predictor(currRow[0],currRow[1],currRow[2],currRow[3],currRow[4],currRow[5],currRow[6], listOfModels, n))

    return anomalies


def processCSV(dir_, timestamp_col, value_col, listOfModels, n, append=True):
    """
    ...
    anomalyCol = pd.DataFrame(processDF(df2, listOfModels, n))
